...
/**
 * REST controller for managing {@link com.mycompany.process.domain.Course}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CourseResource {
    ...
    public ResponseEntity<Course> partialUpdateCourse(
        ...
        Optional<Course> result = courseRepository
            ...
            .map(existingCourse -> {
                ...
                if (course.getOfferId() != null) {
                    existingCourse.setOfferId(course.getOfferId());
                }
    ...
    public List<Course> getAllCourses(@RequestParam(required = false) String filter) {
        if ("simulation-is-null".equals(filter)) {
            log.debug("REST request to get all Courses where simulation is null");
            return StreamSupport
                .stream(courseRepository.findAll().spliterator(), false)
                .filter(course -> course.getSimulation() == null)
