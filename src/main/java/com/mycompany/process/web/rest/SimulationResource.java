...
/**
 * REST controller for managing {@link com.mycompany.process.domain.Simulation}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Simulation> partialUpdateSimulation(
        ...
        Optional<Simulation> result = simulationRepository
            ...
            .map(existingSimulation -> {
                ...
                if (simulation.getModified() != null) {
                    existingSimulation.setModified(simulation.getModified());
                }
                if (simulation.getUserCreation() != null) {
                    existingSimulation.setUserCreation(simulation.getUserCreation());
                }
                if (simulation.getUserCurrent() != null) {
                    existingSimulation.setUserCurrent(simulation.getUserCurrent());
                }
