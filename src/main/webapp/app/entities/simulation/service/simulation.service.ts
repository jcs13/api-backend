...
export class SimulationService {
  ...
  protected convertDateFromClient(simulation: ISimulation): ISimulation {
    return Object.assign({}, simulation, {
      ...
      modified: simulation.modified?.isValid() ? simulation.modified.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((simulation: ISimulation) => {
        ...
        simulation.modified = simulation.modified ? dayjs(simulation.modified) : undefined;
      });
    }
    return res;
  }
